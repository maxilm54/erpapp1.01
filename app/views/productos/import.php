...
<div class="card p-4 shadow col-md-8 mx-auto">
    ...
    <p class="text-muted">Subí un archivo .xlsx con los productos a importar.</p>

    <div class="alert alert-info">
        <strong>Formato esperado del archivo (columnas):</strong>
        <code>nombre</code>, <code>sku</code>, <code>precio_venta</code>, <code>unidad_medida</code> (opcional), <code>barcode</code> (opcional).
        <br>
        <small>La primera fila debe ser el encabezado. La <strong>unidad_medida</strong> se matchea por nombre (ej: "kg", "un", "lt"). Si no coincide, usa la unidad por defecto.</small>
    </div>
...
<div class="card p-4 shadow">
    <h4><i class="bi bi-table"></i> Vista Previa - Importar Productos</h4>
    <?php
    ...
    <form method="POST" class="mt-3">
        ...
        <button type="submit" name="confirmar" value="1" class="btn btn-success">
            <i class="bi bi-check-lg"></i> Confirmar Importación (<?= count($validos) ?> productos)
        </button>
        <?php else: ?>
        <button type="submit" class="btn btn-success" disabled>
            <i class="bi bi-check-lg"></i> No hay productos válidos para importar
        </button>
        <?php endif; ?>
        <a href="<?= BASE_URL ?>/productos/import" class="btn btn-warning">
