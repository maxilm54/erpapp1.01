...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .hdr-centro .estado-borrador { background: #6c757d; }
        .hdr-centro .estado-aprobado { background: #198754; }
...
<div class="header">
    <table class="header-tabla">
        <tr>
            ...
            <td class="hdr-der">
                <div class="doc-numero">N° <?= $presupuesto['id'] ?></div>
                <div class="doc-fecha">Fecha: <?= date('d/m/Y', strtotime($presupuesto['created_at'])) ?></div>
            </td>
        </tr>
    </table>
</div>
...
<div class="aviso-legal">
    <strong>PRESUPUESTO</strong> — Este documento <strong>no constituye comprobante fiscal</strong>.
        ...
        <tr>
            <td style="width:65%; vertical-align:top; padding:0;">
                <div class="dato-fila">
                    ...
                    <span class="dato-valor"><?= htmlspecialchars($presupuesto['cliente']) ?></span>
                </div>
                <div class="dato-fila">
                    <span class="dato-label">Domicilio: </span>
                    <span class="dato-valor"><?= htmlspecialchars($presupuesto['direccion'] ?? '-') ?></span>
                </div>
            </td>
            <td style="width:35%; vertical-align:top; padding:0;">
                <div class="dato-fila">
                    <span class="dato-label">CUIT: </span>
                    <span class="dato-valor"><?= htmlspecialchars($presupuesto['cuit'] ?? '-') ?></span>
                </div>
            </td>
        </tr>
    </table>
...
<!-- DETALLE -->
<div class="seccion-titulo">Detalle del Presupuesto</div>
<?php
    $totalGeneral = 0;
...
<table class="productos-tabla">
    ...
    <tbody>
        <?php $i = 1; foreach ($presupuesto['detalle'] as $d):
            $precio = (float)$d['precio'];
            $cant = (float)$d['cantidad'];
            $sub = $precio * $cant;
        ?>
...
<!-- OBSERVACIONES -->
<?php if (!empty(trim($presupuesto['observaciones'] ?? ''))): ?>
<div class="observaciones">
    <div class="seccion-titulo">Observaciones</div>
    <div class="observaciones-texto"><?= nl2br(htmlspecialchars($presupuesto['observaciones'])) ?></div>
</div>
<?php endif; ?>
...
<div class="footer-fixed">
    ...
    <em>Documento generado automáticamente. Presupuesto — No válido como factura.</em>
</div>
