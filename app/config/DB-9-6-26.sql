...
) ENGINE=InnoDB AUTO_INCREMENT=163 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Tabla de referencia principal del stock';
...
DELIMITER //
CREATE DEFINER=`root`@`localhost` TRIGGER `historico_reserva_mp_trig` AFTER DELETE ON `reservas_materia_prima` FOR EACH ROW BEGIN
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vststock_movstock_materiaprima` AS SELECT msmp.materia_prima_id, mp.nombre,
COALESCE(SUM(
  CASE
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vststock_movstock_producto` AS SELECT msp.producto_id, pd.nombre,
COALESCE(SUM(
  CASE
